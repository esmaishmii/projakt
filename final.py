...
class Calc:
    ...
    def create_widgets(self):
        self.txtDisplay = tk.Entry(self.master, font=('Helvetica', 20, 'bold'),
                                   bg='black', fg='white',
        ...
        for j in range(2, 5):
            for k in range(3):
                self.btn.append(tk.Button(self.master, width=6, height=2,
                                           bg='black', fg='white',
        ...
        self.btnClear = tk.Button(self.master, text=chr(67), width=6,
                                  height=2, bg='powder blue',
                                  font=('Helvetica', 20, 'bold'),
                                  bd=4, command=self.Clear_Entry
                                  )
        self.btnClear.grid(row=1, column=0, pady=1)

        self.btnAllClear = tk.Button(self.master, text=chr(67) + chr(69),
                                     width=6, height=2,
                                     bg='powder blue',
                                     font=('Helvetica', 20, 'bold'),
                                     bd=4,
                                     command=self.All_Clear_Entry
                                     )
        self.btnAllClear.grid(row=1, column=1, pady=1)

        self.btnsq = tk.Button(self.master, text="\u221A", width=6, height=2,
                               bg='powder blue', font=('Helvetica',
                                                        20, 'bold'),
                               bd=4, command=self.squared
                               )
        self.btnsq.grid(row=1, column=2, pady=1)

        self.btnAdd = tk.Button(self.master, text="+", width=6, height=2,
                                bg='powder blue',
                                font=('Helvetica', 20, 'bold'),
                                bd=4, command=lambda: self.operation("add")
                                )
        self.btnAdd.grid(row=1, column=3, pady=1)

        self.btnSub = tk.Button(self.master, text="-", width=6, height=2,
                                bg='powder blue',
                                font=('Helvetica', 20, 'bold'),
                                bd=4, command=lambda: self.operation("sub")
                                )
        self.btnSub.grid(row=2, column=3, pady=1)

        self.btnMul = tk.Button(self.master, text="x", width=6, height=2,
                                bg='powder blue',
                                font=('Helvetica', 20, 'bold'),
                                bd=4, command=lambda: self.operation("multi")
                                )
        self.btnMul.grid(row=3, column=3, pady=1)

        self.btnDiv = tk.Button(self.master, text="/", width=6, height=2,
                                bg='powder blue',
                                font=('Helvetica', 20, 'bold'),
                                bd=4, command=lambda: self.operation("divide")
                                )
        self.btnDiv.grid(row=4, column=3, pady=1)

        self.btnZero = tk.Button(self.master, text="0", width=6, height=2,
                                 bg='black', fg='white',
                                 font=('Helvetica', 20, 'bold'),
                                 bd=4, command=lambda: self.numberEnter(0)
                                 )
        self.btnZero.grid(row=5, column=0, pady=1)

        self.btnDot = tk.Button(self.master, text=".", width=6, height=2,
                                bg='powder blue',
                                font=('Helvetica', 20, 'bold'),
                                bd=4, command=lambda: self.numberEnter(".")
                                )
        self.btnDot.grid(row=5, column=1, pady=1)

        self.btnPM = tk.Button(self.master, text=chr(177), width=6, height=2,
                               bg='powder blue', font=('Helvetica', 20, 'bold'),
                               bd=4, command=self.mathPM
                               )
        self.btnPM.grid(row=5, column=2, pady=1)

        self.btnEquals = tk.Button(self.master, text="=", width=6, height=2,
                                   bg='powder blue',
                                   font=('Helvetica', 20, 'bold'),
                                   bd=4, command=self.sum_of_total
                                   )
...
class MatrixWindow(tk.Toplevel): 
    ...
    def create_widgets(self):
        label_vrste = tk.Label(self, text="Unesite broj vrsta:")
        label_vrste.grid(row=0, column=0)
        self.entry_vrste = tk.Entry(self)
        self.entry_vrste.grid(row=0, column=1)

        label_kolone = tk.Label(self, text="Unesite broj kolona:")
        label_kolone.grid(row=1, column=0)
        self.entry_kolone = tk.Entry(self)
        ...
        self.entry_elementi = tk.Entry(self)
        self.entry_elementi.grid(row=2, column=1)

        kreiraj_button1 = tk.Button(self, text="Kreiraj Prvu Matricu", command=self.kreiraj_matricu1)
        kreiraj_button1.grid(row=3, columnspan=2)

        kreiraj_button2 = tk.Button(self, text="Kreiraj Drugu Matricu", command=self.kreiraj_matricu2)
        kreiraj_button2.grid(row=4, columnspan=2)

        saberi_button = tk.Button(self, text="Saberi Matrice", command=self.saberi_i_prikazi)
        saberi_button.grid(row=5, columnspan=2)

        oduzmi_button = tk.Button(self, text="Oduzmi Matrice", command=self.oduzmi_i_prikazi)
        oduzmi_button.grid(row=6, columnspan=2)

        pomnozi_button = tk.Button(self, text="Pomnoži Matrice", command=self.pomnozi_matrice)
        pomnozi_button.grid(row=7, columnspan=2)

        transponuj_button = tk.Button(self, text="Transponuj Prvu Matricu", command=self.transponuj_i_prikazi)
...
class GeometricCalculator(tk.Tk):
    ...
    def create_widgets(self):
        # Izbor oblika
        tk.Label(self, text="Select Shape:").pack()
        ...
        voice_button = tk.Button(self, text="Use Voice Command", command=self.listen)
        voice_button.pack()
...
class MainMenu(tk.Tk):
    ...
    def create_widgets(self):
        tk.Label(self, text="Odaberite opciju:").pack()
        tk.Button(self, text="Kalkulator", command=self.open_calculator).pack()
        tk.Button(self, text="Matrice", command=self.open_matrix_operations).pack()
        tk.Button(self, text="Geometrija", command=self.open_geometry_calculator).pack()
        tk.Button(self, text="Jednacina", command=self.open_equation_solver).pack()
